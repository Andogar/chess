...
public class Rook extends Piece {
    ...
    // List of valid moves based on a board with tiles from 0-63 for Rook
    private final static int[] POTENTIAL_MOVE_VECTORS = {-8, -1, 1, 8};
    ...
    private static boolean isFirstColumnExclusion(final int currentPosition, final int positionOption) {
        return BoardUtilities.FIRST_COLUMN[currentPosition] && (positionOption == -1);
    }

    private static boolean isEighthColumnExclusion(final int currentPosition, final int positionOption) {
        return BoardUtilities.EIGHTH_COLUMN[currentPosition] && (positionOption == 1);
    }
}
