...
public class Queen extends Piece {
    ...
    // List of valid moves based on a board with tiles from 0-63 for Queen
    private final static int[] POTENTIAL_MOVE_VECTORS = {-9, -8, -7, -1, 1, 7, 8, 9};
    ...
    private static boolean isFirstColumnExclusion(final int currentPosition, final int positionOption) {
        return BoardUtilities.FIRST_COLUMN[currentPosition] && (positionOption == -1 || positionOption == -9 || positionOption == 7);
    }

    private static boolean isEighthColumnExclusion(final int currentPosition, final int positionOption) {
        return BoardUtilities.EIGHTH_COLUMN[currentPosition] && (positionOption == 1 || positionOption == 9 || positionOption == -7);
    }
}
